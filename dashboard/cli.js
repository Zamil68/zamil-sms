/* ═══ cli.js — CLI Insights (Zamil SMS) v3 — exact ranges + smart cache ═══ */
(function(){
'use strict';
  ...
  post('/api/cli/gate',{openAction:'open_insights'},function(g){
    if(g&&g.ok&&g.insights===false){
      ...
      if(p){
        ...
        ov.innerHTML='<div style="max-width:360px;width:100%;text-align:center;background:var(--card,#14161d);border:1px solid var(--border,#262a35);border-radius:24px;padding:36px 26px"><div style="font-family:\'Space Grotesk\',sans-serif;font-size:1.15rem;font-weight:700;margin-bottom:8px;color:var(--text,#e8ecf4)">Access Restricted</div><div style="font-size:.82rem;color:var(--muted,#9aa3b2);line-height:1.6">CLI Insights has been disabled for your account.</div><button onclick="closeCliPage()" style="margin-top:18px;border:0;border-radius:12px;padding:11px 22px;font:inherit;font-weight:700;cursor:pointer;color:#06080f;background:linear-gradient(135deg,var(--accent,#22d3ee),var(--accent2,#818cf8))">Go Back</button></div>';
    ...
    if(p2){
      ...
      if(_list.length&&(Date.now()-_cacheTs)<CACHE_TTL){ renderStats(); cliRenderList(); }
...
window.closeCliPage=function(){
  ...
  if(_refetchT){ clearInterval(_refetchT); _refetchT=null; }
};
...
window.cliRenderList=function(){
  ...
  var data=_list;
  if(q){ data=_list.filter(function(x){ var hay=(x.cli+' '+x.countries.map(function(c){return c.name+' '+(c.range||'');}).join(' ')).toLowerCase(); return hay.indexOf(q)>=0; }); }
  ...
  el.innerHTML=data.map(function(x,i){
    var flags=x.countries.slice(0,4).map(function(c){ return '<span class="cli-flag" title="'+esc(c.range||c.name)+' · '+(c.n||0).toLocaleString()+' SMS">'+c.flag+'</span>'; }).join('');
    ...
    var ctry=(x.countries&&x.countries.length)?('<div class="cli-ctrylist">'+x.countries.map(function(c){
      var label=c.range||c.name||'Unknown';
      return '<div class="cli-ctry" title="'+esc(label)+' · '+(c.n||0).toLocaleString()+' SMS">'
        +'<span class="cli-ctry-flag">'+(c.flag||'🏳️')+'</span>'
        +'<span class="cli-ctry-name">'+esc(label)+'</span>'
